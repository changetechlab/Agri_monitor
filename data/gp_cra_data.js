 ...
 * Source: LGD (Local Government Directory), Govt of India
 */
...
window.GP_CRA_DATA = {
  ...
  gp_list: [
    ...
    /* ══════════════════════════════════════════════════════════
       BLOCK: UKHIMATH (Sub-District: 290)
    ══════════════════════════════════════════════════════════ */
    {
      id: 'gp_ukhimath_01',
      name: 'Ukhimath GP',
    ...
    /* ══════════════════════════════════════════════════════════
       BLOCK: JAKHOLI (Sub-District: 292)
    ══════════════════════════════════════════════════════════ */
    {
      id: 'gp_jakholi_01',
      name: 'Jakholi GP',
    ...
    /* ══════════════════════════════════════════════════════════
       BLOCK: AGASTYAMUNI (Sub-District: 291)
    ══════════════════════════════════════════════════════════ */
    {
    ...
    /* ══════════════════════════════════════════════════════════
       BLOCK: BASUKEDAR (Sub-District: 6334)
    ══════════════════════════════════════════════════════════ */
    {
      id: 'gp_basukedar_01',
      name: 'Basukedar GP',
  ...
  getDistrictStats() {
    return {
      ...
      total_villages: this.gp_list.reduce((s, gp) => s + (gp.village_count || 1), 0),
      total_farmers: this.gp_list.reduce((s, gp) => s + (gp.total_farmers || 0), 0),
      total_agri_ha: this.gp_list.reduce((s, gp) => s + (gp.agri_area_ha || 0), 0),
...
/* -----------------------------------------------------------
   DYNAMICALLY INJECT LGD VILLAGES AS GP DATA (CLEAN & BUG-FREE)
----------------------------------------------------------- */
(function() {
  if (window.DummyData && window.DummyData.villages) {
    ...
    // साफ हिंदी फसलें (Fix for ??????)
    const hillCrops = ['मंडुआ', 'झंगोरा', 'राजमा', 'गहत', 'आलू', 'गेहूं'];
    ...
    lgd.forEach(v => {
      ...
      const avg_ndvi = parseFloat((0.28 + rand * 0.40).toFixed(2));
      const slopes = ['low', 'moderate', 'steep', 'very_steep'];
      const slope = slopes[Math.floor(Math.random() * slopes.length)];
      const drains = ['good', 'moderate', 'poor'];
      const drainage = drains[Math.floor(Math.random() * drains.length)];
      const waters = ['seasonal', 'scarce'];
      const water_avail = waters[Math.floor(Math.random() * waters.length)];
      
      const hazards = [];
      if (avg_ndvi < 0.35) hazards.push('drought');
      if (slope === 'steep' || slope === 'very_steep') hazards.push('landslide');
      if (hazards.length === 0) hazards.push('drought');

      // स्लोप का प्रतिशत ब्रेकअप (Donut Chart Fix)
      let slopeBreakup = { flat: 20, gentle: 35, moderate: 30, steep: 12, very_steep: 3 };
      if (slope === 'steep' || slope === 'very_steep') {
        slopeBreakup = { flat: 10, gentle: 20, moderate: 35, steep: 25, very_steep: 10 };
      }
      ...
      window.GP_CRA_DATA.gp_list.push({
        ...
        // एरिया और किसान
        total_area_ha: Math.floor(250 + Math.random() * 200),
        agri_area_ha: Math.floor(60 + Math.random() * 100),
        total_farmers: Math.floor(80 + Math.random() * 200),
        ...
        land_use: {
          forest_pct: Math.floor(35 + Math.random() * 30),
          agri_pct: Math.floor(25 + Math.random() * 20),
          barren_pct: 12,
          settlement_pct: 5
        },
