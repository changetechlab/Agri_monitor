 ...
 *
 * DATA SOURCE STATUS
 * Current source: EOX Sentinel-2 Cloudless 2020 (pre-rendered RGB tiles)
 * Individual spectral bands (B3,B4,B5,B8,B8A,B11) are NOT available.
 * Real pixel-level index computation is DISABLED until GEE/TiTiler is configured.
 *
 * To enable real raster processing:
 *   window.AgriConfig.GEE_TILE_URL = '<your GEE tile endpoint>'  OR
 *   window.AgriConfig.NDVI_TILE_URL = '<your TiTiler instance>'
 *
 * Future data flow:
 *   GEE/TiTiler -> S2 bands -> Cloud mask -> Index Engine -> Analytics
 *
...
window.SatelliteEngine = (() => {
  ...
  // ============================================================
  // PURE CALCULATION FUNCTIONS (ready for GEE/TiTiler integration)
  // ============================================================
  ...
  // ============================================================
  // TILE ROUTING (future-ready)
  // ============================================================
  function getTileUrl(indexType, date) {
  ...
  function getIndexCapability(indexType) {
    const cfg = INDEX_CONFIG[indexType];
    if (!cfg) return { available: false, reason: 'Unknown index' };
    const appCfg  = window.AgriConfig || {};
    const hasGEE  = appCfg.GEE_TILE_URL && appCfg.GEE_TILE_URL !== 'null';
    const hasTiti = appCfg.NDVI_TILE_URL && !appCfg.NDVI_TILE_URL.includes('titiler.xyz');
    if (cfg.dataStatus === 'available') return { available: true, reason: 'Simulation; real data with GEE/TiTiler' };
    if (hasGEE || hasTiti) return { available: true, reason: 'Band-capable source detected' };
    return {
      available: false,
      reason: 'Requires S2 bands (' + cfg.requiredBands.join(', ') + '). Configure GEE_TILE_URL or NDVI_TILE_URL.',
    };
  }
  ...
  function _simNDWI(seed, month) {
    ...
    const base = monsoon.includes(month) ? 0.15 : -0.10;
    return Math.max(-0.8, Math.min(0.8, base + (seed % 5) * 0.04 - 0.08));
  }
